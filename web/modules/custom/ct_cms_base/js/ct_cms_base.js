...
((Drupal) => {
  Drupal.behaviors.ysBase = {
    ...
    initCounterBlock(context) {
      const counterBlocks = context.querySelectorAll('[data-ct-cms-base-counter]');

      counterBlocks.forEach(function processBlock(block) {
        // Skip if already processed.
        if (block.classList.contains('ct-cms-base-counter-processed')) {
          return;
        }

        block.classList.add('ct-cms-base-counter-processed');
